...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()//항상 물리적인 움직임으로 FixedUpdate 사용함
    {
        ...
        //죽었거나 맞는 상태면 이동하지 않기
        if (!canMove || !isLive || anim.GetCurrentAnimatorStateInfo(0).IsName("Hit")) return;
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        else
        {
            ...
            Boss forRemove = GetComponent<Boss>();
            if (forRemove != null)
            {
                this.transform.localScale = Vector3.one;
                //보스 삭제
                GameObject box = GameManager.instance.pool.Get("TreasureBox");
                box.transform.position = this.transform.position;
                Destroy(forRemove);
            }
            else
            {
                GameObject coin = GameManager.instance.pool.Get("Coin");
                coin.transform.position = this.transform.position;
                coin.GetComponent<Coin>().Init((int)maxHealth);
            }
